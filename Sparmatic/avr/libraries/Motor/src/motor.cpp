...
motor::motor() : 
    _position(0),
    _positionValveOpen(0),
    _positionValveClosed(0),
    _stopSource(0),
    _direction(0),
    _targetPosition(-1),
    _timeout(95),
...
/// 
/// 
void motor::stopTimeout()
{
	_stopSource |= STOP_TIMEOUT;
	stopMove();
}
...
/// 
/// 
void motor::move(int8_t dir)
{
	MOTOR_SENSE_ON;
	MOTOR_STOP;
	MOTOR_DIR_OUT;
	_stopSource = STOP_NULL;
	enableTimeout(stopTimeout, 255);	/* first timeout long to allow startup */
	switch(dir)
	{
		case DIR_OPEN:
			MOTOR_OPEN;
 ...
 */
void motor::moveTo(uint8_t valve)
{
	int16_t newPosition = _positionValveClosed + ((_positionValveOpen - _positionValveClosed) * valve) / 255;
	_targetPosition = newPosition;
	if(newPosition > _position + MOTOR_TARGET_HYSTERESIS)
	{
		_targetPosition -= MOTOR_TARGET_STOP_EARLY;
		move(DIR_OPEN);
	}
	else if(newPosition < _position - MOTOR_TARGET_HYSTERESIS)
	{
		_targetPosition += MOTOR_TARGET_STOP_EARLY;
		move(DIR_CLOSE);
	}
 ...
 */
uint8_t motor::fullOpen(void)
{
	_timeout = MOTOR_SPEED_BLOCK_OPEN;
	_currentLimit = MOTOR_CURRENT_BLOCK_OPEN;
	_position = 0;
	_targetPosition = -1;
	move(DIR_OPEN);
	while(isRunning())
		;
	_position = MOTOR_POSITION_MAX;
	if(_stopSource & ~(STOP_CURRENT | STOP_TIMEOUT))
...
uint8_t motor::adapt()
{
	...
	_timeout = MOTOR_SPEED_BLOCK;
	_currentLimit = MOTOR_CURRENT_BLOCK;
	
	move(DIR_CLOSE);
	/* let vent start moving */
	while(isRunning() && _position > MOTOR_POSITION_MAX - 11)
		;
	...
	while(isRunning() && getCurrent() > (currentNormal - MOTOR_CURRENT_VALVE_DETECT));
	
	_positionValveOpen = _position;
	...
	_positionValveClosed = _position;
	
	/* check for min. vent range, we may have an error in detection */
	if(_positionValveOpen - _positionValveClosed < MOTOR_VENT_RANGE_MIN)
		return 1;
...
uint8_t motor::step()
{
	...
	if(((_direction == DIR_OPEN) && state) || ((_direction == DIR_CLOSE) && !state)) 
	{
		setTimeout(_timeout);
		_position += _direction;
		
		// limits
		if(_position < 0 || _position > MOTOR_POSITION_MAX) 
		{
			_stopSource |= STOP_POSITION;
			stopMove();
		}
		
		// target
		if(	_targetPosition >= 0
				&& (   (_direction == DIR_OPEN && _position >= _targetPosition)
						|| (_direction == DIR_CLOSE && _position <= _targetPosition)
						)
			)
		{
			_stopSource |= STOP_TARGET;
			_targetPosition = -1;
			stopMove();
		}
		return 1;
...
/// 
/// 
uint8_t motor::isRunning(void)
{
	return (_direction != DIR_STOP);
}
